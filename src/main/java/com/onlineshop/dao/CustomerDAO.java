...
public class CustomerDAO implements CustomerDAOInterface {
	...
	public List<Orders> findTotalProductDAO(Object attribute, int order_no) {
		List<Orders> ll=new ArrayList<Orders>();
		try {
            //Connecting database connection and querying in the database
		 Connection con=DatabaseConnection.getConnection();
	...
	public int reviewDAO(review rev) {
		// TODO Auto-generated method stub
		int i=0;
		try{
		Connection con=DatabaseConnection.getConnection();
		 PreparedStatement ps=con.prepareStatement("insert into reviews(product_id,user_id,rating,review_text) values(?,?,?,?)");
		 ps.setInt(1, rev.getProductId());
		 ps.setInt(2, rev.getUserId());
		 ps.setInt(3, rev.getRating());
		 ps.setString(4, rev.getReviewText());
		 i=ps.executeUpdate();
	...
	public int complaintDAO(complaint com) {
		// TODO Auto-generated method stub
		int i=0;
		try{
		Connection con=DatabaseConnection.getConnection();
		 PreparedStatement ps=con.prepareStatement("insert into complaints(user_id, retailer_id, complaint_text) values(?,?,?)");
		 ps.setInt(1, com.getUserId());
		 ps.setInt(2, com.getRetailerId());
		 ps.setString(3, com.getComplaintText());
